...
import { Keyboard, Clock, RotateCcw, Trophy, Zap, ChevronDown, Menu, X } from 'lucide-react'
...
const NAV_SECTIONS = [
  { id: 'A1', sets: ['A1 Basics', 'Numbers'] },
  { id: 'A2', sets: ['A2 Phrases'] },
  { id: 'B1', sets: ['B1 Vocabulary'] },
]
...
function GamePanel({ category, onCategoryChange }) {
      ...
      {gameState === 'idle' && (
        <article>
          <div className="flex flex-wrap gap-2 mb-4">
            <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${LEVEL_COLORS[bank.level]}`}>
              {bank.level}
            </span>
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 mb-6 pb-5 border-b border-gray-200 dark:border-gray-700">
            <span className="text-sm text-gray-500 dark:text-gray-400">60 second race · up to +60 XP</span>
            {bestScore > 0 && (
              <span className="flex items-center gap-1.5 text-sm font-bold text-amber-600 dark:text-amber-400">
                <Trophy className="w-4 h-4" /> Best: {bestScore} pts
              </span>
            )}
          </div>

          <p className="text-gray-700 dark:text-gray-300 mb-8 leading-relaxed text-[15px]">{bank.description}</p>
          ...
          {/* Word preview */}
          <h2 className="text-base font-bold text-gray-900 dark:text-white mb-3">Words in this set</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-8">
            {bank.words.map((w, i) => (
              <div key={i} className="bg-white dark:bg-dark-warm-200 rounded-lg px-3 py-2 border border-gray-100 dark:border-gray-700">
                <p className="text-sm font-semibold text-gray-900 dark:text-white">{w.fr}</p>
                <p className="text-xs text-gray-500 dark:text-gray-400">{w.en}</p>
              </div>
            ))}
          </div>
          ...
          <button
            onClick={startGame}
            className="flex items-center gap-3 px-8 py-4 bg-burgundy-600 text-cream-50 rounded-2xl font-bold text-lg hover:bg-burgundy-700 transition-colors shadow-lg"
          >
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh] text-center px-4 py-16">
      ...
      <p className="text-gray-600 dark:text-gray-300 max-w-md leading-relaxed">
        See a French word — type its English translation as fast as you can! Build a streak for bonus XP.
      </p>
      <p className="text-sm text-gray-400 dark:text-gray-500 mt-8 flex items-center gap-2">
        <span>←</span>
        <span>Pick a word set from the sidebar to begin</span>
      </p>
    </div>
  )
}
...
export default function TypingRace() {
  ...
  const [openSections, setOpenSections] = useState({ A1: true, A2: true, B1: true })
              ...
              <div key={id} className="mb-1">
                ...
                >
                  <span className="flex items-center gap-2.5">
                    ...
                    <span className="text-gray-700 dark:text-gray-200">{sets.length} {sets.length === 1 ? 'set' : 'sets'}</span>
                  </span>
                  <ChevronDown
                    size={14}
          ...
          <div className="lg:hidden sticky top-20 z-20 bg-white dark:bg-dark-warm-100 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center gap-3">
            ...
            {activeCategory && (
              <>
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400 truncate">{activeCategory}</span>
              </>
            )}
          </div>
          ...
          <div className="px-6 py-10 lg:px-12 lg:py-10 max-w-2xl">
            <AnimatePresence mode="wait">
              {!activeCategory ? (
                ...
                >
                  <GamePanel category={activeCategory} onCategoryChange={setActiveCategory} />
                </motion.div>
              )}
            </AnimatePresence>
          </div>
